...
function initLanding() {
  ...
  function elegirRol(rol) {
    ...
    if (rolCliente) {
      rolCliente.classList.toggle("active", rol === "cliente");
      rolCliente.classList.toggle("activo", rol === "cliente");

      if (rol === "cliente") {
        rolCliente.style.backgroundColor = "#4a80e8";
        rolCliente.style.color = "#ffffff";
      } else {
        rolCliente.style.backgroundColor = "transparent";
        rolCliente.style.color = "#7b93a7";
      }
    }

    if (rolTrabajador) {
      rolTrabajador.classList.toggle("active", rol === "trabajador");
      rolTrabajador.classList.toggle("activo", rol === "trabajador");

      if (rol === "trabajador") {
        rolTrabajador.style.backgroundColor = "#4a80e8";
        rolTrabajador.style.color = "#ffffff";
      } else {
        rolTrabajador.style.backgroundColor = "transparent";
        rolTrabajador.style.color = "#7b93a7";
      }
    }

    if (camposTrabajador) {
      camposTrabajador.style.display =
        rol === "trabajador" ? "block" : "none";
    }
  }

  if (rolCliente) {
    rolCliente.addEventListener("click", () => {
      elegirRol("cliente");
    });
  }

  if (rolTrabajador) {
    rolTrabajador.addEventListener("click", () => {
  ...
  if (descripcion && contadorDesc) {
    descripcion.addEventListener("input", () => {
      contadorDesc.textContent =
        `${descripcion.value.length} / 200`;
    });
  }
  ...
  function mostrarMensaje(texto, tipo = "error") {
    if (mensajeArea) {
      mensajeArea.innerHTML =
        `<span class="mensaje mensaje-${tipo}">${texto}</span>`;
    }
  }
  ...
  if (formRegistro) {
    formRegistro.addEventListener("submit", (evento) => {
      ...
      const nuevoUsuario = {
        ...
        rol: rolSeleccionado,
        oficio:
          document.getElementById("reg-oficio")?.value.trim() || "",
        categoria:
          document.getElementById("reg-categoria")?.value || "",
        zona:
          document.getElementById("reg-zona")?.value.trim() || "",
        precioPorHora:
          precioInput ? Number(precioInput) : null,
        descripcion:
  ...
  /* Estado inicial: Cliente seleccionado */
...
function initPerfil() {
  ...
  if (usuario.rol === "trabajador") {
    const datosTrabajador =
      document.getElementById("datos-trabajador");
    ...
    const elemOficio =
      document.getElementById("perfil-oficio");

    const elemCategoria =
      document.getElementById("perfil-categoria");

    const elemZona =
      document.getElementById("perfil-zona");

    const elemPrecio =
      document.getElementById("perfil-precio");

    const elemDesc =
      document.getElementById("perfil-descripcion");

    if (elemOficio) {
      elemOficio.textContent =
        usuario.oficio || "—";
    }

    if (elemCategoria) {
      elemCategoria.textContent =
        usuario.categoria || "—";
    }

    if (elemZona) {
      elemZona.textContent =
        usuario.zona || "—";
    }

    if (elemPrecio) {
      elemPrecio.textContent =
        usuario.precioPorHora
          ? usuario.precioPorHora
          : "—";
    }
...
function initPerfilTrabajador() {
  const profesionalesDemo = [
    {
      ...
      descripcion:
        "Electricista matriculado con más de 10 años de experiencia en trabajos residenciales y comerciales en Tucumán Capital.",
      zona:
...

    {
      ...
      descripcion:
        "Especialista en reparación de pérdidas, instalación de sanitarios y mantenimiento preventivo de cañerías.",
      zona:
        "Tucumán Zona Sur y Centro",
      disponibilidad:
        "🟢 Disponible hoy",
      disponible: true
    }
  ...
  const prof =
    profesionalesDemo.find(
      (p) => p.id === profId
    ) || profesionalesDemo[0];

  const elemIniciales =
    document.getElementById("prof-iniciales");

  const elemEstadoPunto =
    document.getElementById("prof-estado-punto");

  const elemNombre =
    document.getElementById("prof-nombre");

  const elemOficio =
    document.getElementById("prof-oficio");

  const elemEstrellas =
    document.getElementById("prof-estrellas");

  const elemRating =
    document.getElementById("prof-resumen-rating");

  const elemPrecio =
    document.getElementById("prof-precio");

  const elemDistancia =
    document.getElementById("prof-distancia");

  const elemLlegada =
    document.getElementById("prof-llegada");

  const elemEsp =
    document.getElementById("prof-especialidades");

  const elemDesc =
    document.getElementById("prof-descripcion");

  const elemZona =
    document.getElementById("prof-zona");

  const elemDisp =
    document.getElementById("prof-disponibilidad");

  if (elemIniciales) {
    elemIniciales.textContent =
      prof.iniciales;
  }
  ...
  if (elemEstrellas) {
    elemEstrellas.textContent =
      prof.estrellas;
  }

  if (elemRating) {
    elemRating.textContent =
      `${prof.ratingNum} de 5 · ${prof.resenas} reseñas`;
  }
  ...
  if (elemDistancia) {
    elemDistancia.textContent =
      prof.distancia;
  }

  if (elemLlegada) {
    elemLlegada.textContent =
      prof.llegada;
  }
  ...
  if (elemEsp) {
    elemEsp.innerHTML =
      prof.especialidades
        .map(
          (e) =>
            `<span class="tag-especialidad">${e}</span>`
        )
        .join("");
  }
  ...
  if (btnContratar) {
    btnContratar.addEventListener("click", () => {
      window.location.href =
        `crearpedido.html?profesional=${encodeURIComponent(
          prof.nombre
        )}`;
    });
  }
...
function initHistorialTrabajador() {
  ...
  let pedidos =
    JSON.parse(
      localStorage.getItem("oficioya-pedidos")
    ) || [
      {
        id: 1,
        clienteNombre: "Juana Torres",
        direccion:
          "Av. Mate de Luna 2400, Tucumán",
        fecha:
          "18/09/2026 - 15:30 hs",
        descripcion:
          "Necesito revisar el tablero eléctrico por saltos continuos de la térmica.",
...

      {
        ...
        direccion:
          "Barrio Sur, Jujuy 450",
        fecha:
          "20/09/2026 - 10:00 hs",
        descripcion:
          "Instalación de ventilador de techo y luminarias LED.",
        estadoSlug: "aceptado",
        estadoText: "Aceptado"
      }
    ];
      ...
      li.innerHTML = `
        <article class="tarjeta-pedido card p-3 h-100 shadow-sm">
          ...
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <span class="estado estado-${p.estadoSlug}">
              ${p.estadoText}
            </span>
          </div>
            ...
            ${
              p.estadoSlug === "pendiente"
                ? `
                  ...
                  <button
                    class="btn btn-fantasma btn-chico w-100 text-danger border-danger-subtle btn-rechazar"
                    data-id="${p.id}">
                    Rechazar
                  </button>
                `
  ...
  function cambiarEstadoPedido(
    ...
    pedidos = pedidos.map((p) => {
      if (p.id === Number(id)) {
        return {
          ...p,
          estadoSlug: nuevoEstado,
          estadoText: nuevoTexto
        };
      }
  ...
  function asignarEventosAcciones() {
    ...
    document
      .querySelectorAll(".btn-finalizar")
      .forEach((b) => {
        b.addEventListener("click", () => {
          cambiarEstadoPedido(
            b.dataset.id,
...
function initHistorial() {
  ...
  let pedidos =
    JSON.parse(
    ...
    ) || [
      {
        ...
        oficio: "Electricista",
        direccion:
          "Av. Mate de Luna 2400",
        fecha:
          "18/09/2026 - 15:30 hs",
        descripcion:
          "Revisión de tablero eléctrico por saltos de la térmica.",
...

      {
        ...
        fecha:
          "20/09/2026 - 10:00 hs",
        descripcion:
          "Reparación de pérdida de agua debajo de la bacha.",
        estadoSlug: "en-curso",
        estadoText: "En curso",
        calificacion: 0
      }
  ...
  function asignarEventosCalificacion() {
    document
      .querySelectorAll(".estrellas-calificar")
      .forEach((contenedor) => {
        const pedidoId =
          Number(contenedor.dataset.id);
...
function initCrearPedido() {
  ...
  const contadorDesc =
    document.getElementById("contador-desc");
  ...
  const mensajeArea =
    document.getElementById("mensaje-area");
  ...
  if (formPedido) {
    formPedido.addEventListener("submit", (e) => {
      ...
      const nuevoPedido = {
        id: Date.now(),
        clienteNombre:
          usuario.nombre || "Cliente Anónimo",
        profesionalNombre:
          nombreProfesional,
      ...
      if (mensajeArea) {
        mensajeArea.innerHTML = `
          <div class="alert alert-success py-2 small mb-0">
            ¡Pedido enviado con éxito! Redirigiendo a tus pedidos...
          </div>
        `;
      }
